...
GLOB_BLACK_LIST = ("api_sym_lookup_table.h", "extra/full_trace.h")

SERVER_FN_SKIP = ("_init", "_quit")
...
def parse_function_with_comments(cursor: cc.Cursor, filename: str):
    ...
    if len(api_list) != 1:
        error.error(
            f"Function {cursor.spelling} [{filename}:{cursor.location.line}] has multiple api tags"
        )
        return
    ...
    if len(api_config_list) > 1:
        error.error(
            f"Function {cursor.spelling} [{filename}:{cursor.location.line}] has multiple api_config tags"
        )
        return
    ...
    if len(api_config_list) != 0:
        try:
            ...
            error.error(
                f"Function {cursor.spelling} [{filename}:{cursor.location.line}] has invalid api_config tag"
            )
...
def parse_struct_decl(
    cursor: cc.Cursor, generator_mode: str, config: dict
) -> ParseResult:
    result = ParseResult()
    ...
    if generator_mode == "":
        ...
        if fn_prefix == None:
            error.error(
                f"[{cursor.location.file}:{cursor.location.line}] Missing fn_prefix in api_config"
            )

        init_method = config.get("init_method")
        if init_method == None:
            error.error(
                f"[{cursor.location.file}:{cursor.location.line}] Missing init_method in api_config"
            )
        ...
        error.error(
            f"[{cursor.location.file}:{cursor.location.line}] Unknown generator mode: {generator_mode}"
        )
...
def parse_typedef_decl(cursor: cc.Cursor) -> ParseResult:
    ...
    if cursor.underlying_typedef_type.kind == cc.TypeKind.POINTER:
        ...
        if (
            ...
            function = TypedefFunctionPointer(
                cursor.spelling,
            ...
            result.add_typedef_function(function)
            # result.add_function(function)
            return result

        # Pointer to struct
        elif pointee.kind == cc.TypeKind.ELABORATED:
            elaborated = pointee.get_declaration()
            if elaborated.kind == CK.STRUCT_DECL:
                result.add_typedef_struct_pointer(
            ...
            error.error(
                f"Invalid typedef {cursor.spelling} [{cursor.location.file}:{cursor.location.line}] (1)"
            )
        ...
        # typedef u64 UID; (u64 is another typedef to uint64_t)
            ...
            result.add_typedef(
                Typedef(
                    cursor.spelling,
                    elaborated.underlying_typedef_type.spelling,
                    str(cursor.location.file),
                    int(cursor.location.line),
                    cursor.raw_comment,
                )
            ...
            print(elaborated.kind)
            error.error(
                f"Invalid typedef {cursor.spelling} [{cursor.location.file}:{cursor.location.line}] (2)"
            )
    # typedef char name
    elif cursor.underlying_typedef_type.kind in (
        cc.TypeKind.CHAR_S,
        cc.TypeKind.UCHAR,
        cc.TypeKind.SHORT,
        cc.TypeKind.USHORT,
        ...
        cc.TypeKind.LONG,
        cc.TypeKind.ULONG,
        cc.TypeKind.LONGLONG,
        cc.TypeKind.ULONGLONG,
        cc.TypeKind.FLOAT,
        cc.TypeKind.DOUBLE,
        cc.TypeKind.VOID,
    ):
        ...
        print(cursor.underlying_typedef_type.kind)
        error.error(
            f"Invalid typedef {cursor.spelling} [{cursor.location.file}:{cursor.location.line}] (M)"
        )
